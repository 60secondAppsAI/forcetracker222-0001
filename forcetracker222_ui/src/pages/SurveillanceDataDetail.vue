					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateSurveillanceData()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div id="app">
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2SurveillanceDatas = true">Add</base-button>
								              <modal :show.sync="modal2SurveillanceDatas">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Surveillance Data Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="SurveillanceDataId" type="text" placeholder="Enter SurveillanceDataId" v-model="surveillanceDataDetails.surveillanceDataId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Observed Activity</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="ObservedActivity" type="text" placeholder="Enter ObservedActivity" v-model="surveillanceDataDetails.observedActivity"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Timestamp</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<datepicker v-model="surveillanceDataDetails.timestamp"  placeholder="Select Date" ></datepicker>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Reported By</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="ReportedBy" type="text" placeholder="Enter ReportedBy" v-model="surveillanceDataDetails.reportedBy"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  data() {
    return {
      ...
      modal2SurveillanceDatas: false,
      surveillanceDataDetails: null,
  ...
  methods: {
    ...
    async handleAddSubmitted() {
      this.modal2SurveillanceDatas = false;
    ...
    async updateSurveillanceData() {

      const jsonData = JSON.stringify(this.surveillanceDataDetails);
      const res = await SurveillanceDataService.update(jsonData);
...
//        this.getAllSurveillanceDatas();
    ...
    async getSurveillanceDataDetails() {
      try {
			let response = await SurveillanceDataService.get(this.surveillanceDataId);
			this.surveillanceDataDetails = response.data;
 ...
 formatDate(dateString) {
  ...
  mounted() {
    this.getSurveillanceDataDetails();
  },
  computed: {
  },
